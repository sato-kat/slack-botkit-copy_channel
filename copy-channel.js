...
controller.hears('cpc ([a-z0-9_\-]+)','direct_message,direct_mention,mention', function(bot, message) {
	...
	bot.api.channels.info({channel:channelName}, function(err,res){
		if(err){
			bot.api.groups.info({channel:channelName,token:usertoken}, function(err,res){
				if(err){
					bot.reply(message, "groups.info:something is wrong... err:" + err);
				}else{
					var members = res.group.members;
					bot.api.groups.create({name:newChannelName,token:usertoken},function(err,res){
						if(err){
							bot.reply(message, "groups.create:something is wrong... err:" + err);
						}else{
							var channelId=res.group.id;
							for(var x in members){
								bot.api.groups.invite({channel:channelId,user:members[x],token:usertoken},function(err,res){});
							}
			...
			bot.api.channels.create({name:newChannelName,token:usertoken},function(err,res){
				if(err){
					bot.reply(message, "channels.create:something is wrong... err:" + err);
				}else{
					var channelId=res.channel.id;
					for(var x in members){
						bot.api.channels.invite({channel:channelId,user:members[x],token:usertoken},function(err,res){});
					}
